...
for video in "$input_dir"/*.$video_extension; do
  if [ -f "$video" ]; then
    ...
    # ffmpeg -i "$video" -vf "scale=-2:720" -r 30 -c:v libx264 -crf 23 -preset veryfast -c:a aac -b:a 256k "$temp_video"
    ffmpeg -i "$video" -c:v libx264 -c:a aac -b:a 256k "$temp_video"
